import {PAIR_INIT_CODE_HASH} from '@arkswap/abis';
import {type Address, encodePacked, getAddress, keccak256} from 'viem';

/**
 * Client-side mirror of ArkSwapLibrary.
 *
 * These functions render quotes, price impact and minimum-received BEFORE a
 * transaction is sent. They are NOT the source of truth for execution: every
 * swap is bounded on-chain by `amountOutMin` / `amountInMax`, so a stale local
 * quote can cost gas but never funds beyond the user's slippage tolerance
 * (llm.txt s42). The API is never the transaction authority either (llm.txt s37).
 *
...
/**
 * CREATE2 pair address, mirroring `ArkSwapLibrary.pairFor`.
 *
 * PAIR_INIT_CODE_HASH is generated from the compiled ArkSwapPair by
 * scripts/generate-abis.mjs, so this can never drift from the contracts the way
 * a hand-copied constant would (llm.txt s7).
 */
...
/** Lower bound for an exact-input swap. Never call with zero slippage intent. */
...
/**
 * Price impact in basis points: how far execution sits below the mid price.
...
export const PRICE_IMPACT_WARN_BPS = 100n;
export const PRICE_IMPACT_HIGH_BPS = 300n;
export const PRICE_IMPACT_BLOCK_BPS = 1500n;
