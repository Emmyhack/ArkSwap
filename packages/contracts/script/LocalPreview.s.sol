...
contract LocalPreview is ArkScript {
    ...
    function run() external {
        ...
        uint256 pk = vm.envOr("LOCAL_PRIVATE_KEY", uint256(0));
        ...
        ITokenOps(musdc).approve(router, type(uint256).max);
        ITokenOps(musdt).approve(router, type(uint256).max);

        IRouterSeed(router).addLiquidityETH{value: 2_000 ether}(musdc, 2_000e6, 0, 0, me, block.timestamp + 1 hours);
        IRouterSeed(router).addLiquidityETH{value: 1_000 ether}(musdt, 1_000e6, 0, 0, me, block.timestamp + 1 hours);
        IRouterSeed(router).addLiquidity(musdc, musdt, 100_000e6, 100_000e6, 0, 0, me, block.timestamp + 1 hours);
